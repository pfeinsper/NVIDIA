...
def osd_sink_pad_buffer_probe(pad,info,u_data):
    ...
    class Sign:
        def __init__(self, sign_id, label,xl,xr,yt,yb):
            self.label = label
        ...
        def calcArea(self):
            return abs(self.x_right - self.x_left)* abs(self.y_bottom - self.y_top)
    ...
    y_bottom = 0 
    area = 0 
    detected_list = [Sign(-1,None,-1,-1,-1,-1)]
    roadsign_id = -1
    roadsign_detected = -1
    ...
    while l_frame is not None:
        ...
        while l_obj is not None:
            try:
                ...
                detected_list.append(Sign(int(obj_meta.object_id),\
        ...
        largest_obj = max([sign for sign in detected_list], key=attrgetter('area'))
        if largest_obj.area > 38000:
            roadsign_detected = sign_index[largest_obj.label]
            roadsign_id = largest_obj.sign_id
